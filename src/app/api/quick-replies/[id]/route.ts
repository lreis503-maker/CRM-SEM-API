...
export async function PATCH(
  ...
  if (typeof body.title === 'string') {
    ...
    if (!title) return NextResponse.json({ error: "O título não pode estar vazio" }, { status: 400 })
    update.title = title
  }
    ...
    if (body.kind === 'interactive') {
      ...
      if (!text.trim()) {
        return NextResponse.json(
          { error: "O texto da mensagem é obrigatório para respostas rápidas de texto" },
          { status: 400 },
        )
      }
