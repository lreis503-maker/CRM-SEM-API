...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    if (title !== undefined && !title) {
      return NextResponse.json({ error: "O título não pode estar vazio" }, { status: 400 })
    }
    ...
    if (content !== undefined) {
      ...
      if (corrupt) {
        return NextResponse.json({
          success: true,
          warning:
            "Atualizado apenas com busca por palavras-chave. Não foi possível descriptografar sua chave de vetorização. Verifique ENCRYPTION_KEY e informe a chave novamente.",
        })
      }
    }
