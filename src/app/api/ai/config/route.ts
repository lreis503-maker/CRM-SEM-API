...
export async function POST(request: Request) {
  try {
    ...
    if (!body || typeof body !== 'object') return bad("Corpo da solicitação inválido")
    ...
    if (rawHandoff) {
      const { data: member } = await supabase
        ...
        .maybeSingle()
      if (!member) return bad("O atendente de transferência deve ser membro desta conta")
    ...
    if (rawKey) {
      ...
      try {
        ...
        return bad("Não foi possível descriptografar a chave de API salva. Informe a chave novamente.")
      }
    } else {
    ...
    if (rawEmbeddingsKey) {
      try {
        ...
        if (err instanceof AiError) {
          return NextResponse.json(
            { error: `Chave de vetorização: ${err.message}`, code: err.code },
            { status: 400 },
          )
        }
        console.error('[ai/config POST] embeddings validation error:', err)
        return bad("Não foi possível validar a chave de vetorização.")
      }
    }
