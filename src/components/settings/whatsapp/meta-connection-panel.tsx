...
/**
 * The Meta Cloud API connection form and diagnostics.
 *
 * Extracted verbatim from `whatsapp-config.tsx` when UAZAPI was added:
 * same state, same requests, same copy. Only the surrounding
 * `<section>` and panel heading moved to the parent, which now also owns
 * the provider choice. Keeping this file behaviour-identical is what
 * makes "Meta keeps working exactly as before" checkable.
 */
export function MetaConnectionPanel() {
  ...
  const [webhookSecurity, setWebhookSecurity] = useState<{
    configured: boolean;
  } | null>(null);
