...
export function validateInteractivePayload(
  ...
  if (p.body.length > INTERACTIVE_LIMITS.bodyMaxLength) {
    return fail(
      `O texto da mensagem excede o limite de ${INTERACTIVE_LIMITS.bodyMaxLength} caracteres.`,
    )
  }
  ...
  if (p.kind === 'buttons') {
    ...
    if (buttons.length > INTERACTIVE_LIMITS.maxButtons) {
      return fail(
        `Uma mensagem permite no máximo ${INTERACTIVE_LIMITS.maxButtons} botões.`,
      )
    }
    ...
    for (const b of buttons) {
      ...
      if (typeof b.title !== 'string' || b.title.trim() === '') {
        return fail("Cada botão precisa de um texto.")
      }
      if (b.title.length > INTERACTIVE_LIMITS.buttonTitleMaxLength) {
        return fail(
          `O texto do botão "${b.title}" excede o limite de ${INTERACTIVE_LIMITS.buttonTitleMaxLength} caracteres.`,
        )
      }
    }
    return ok()
  ...
  if (p.kind === 'list') {
    ...
    if (list.button_label.length > INTERACTIVE_LIMITS.buttonTitleMaxLength) {
      return fail(
        `O texto do botão da lista excede o limite de ${INTERACTIVE_LIMITS.buttonTitleMaxLength} caracteres.`,
      )
    }
    ...
    for (const section of list.sections) {
      if (!section || !Array.isArray(section.rows)) {
        return fail("Cada seção da lista precisa de opções.")
      }
      for (const row of section.rows) {
        total++
        if (!row || typeof row.id !== 'string' || row.id.trim() === '') {
          return fail("Cada opção da lista precisa de um ID.")
        }
        if (seen.has(row.id)) {
          return fail(`ID de opção de lista duplicado: "${row.id}".`)
        }
        seen.add(row.id)
        if (typeof row.title !== 'string' || row.title.trim() === '') {
          return fail("Cada opção da lista precisa de um título.")
        }
        if (row.title.length > INTERACTIVE_LIMITS.listRowTitleMaxLength) {
          return fail(
            `O título da opção "${row.title}" excede o limite de ${INTERACTIVE_LIMITS.listRowTitleMaxLength} caracteres.`,
          )
        }
        if (
          row.description &&
          ...
          return fail(
            `A descrição da opção excede o limite de ${INTERACTIVE_LIMITS.listRowDescriptionMaxLength} caracteres.`,
          )
        }
      }
    }
    if (total < 1) return fail("Adicione pelo menos uma opção à lista.")
    if (total > INTERACTIVE_LIMITS.maxListRowsTotal) {
      return fail(
        `Uma lista permite no máximo ${INTERACTIVE_LIMITS.maxListRowsTotal} opções no total.`,
      )
    }
    return ok()
  }

  return fail("A mensagem interativa deve conter botões de resposta ou uma lista.")
}
